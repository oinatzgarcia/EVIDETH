...
Tests unitarios de la función verify_ecdsa_signature().

Importa desde app.utils.crypto (módulo puro, sin dependencias de BD).
No requiere BD, ffmpeg, psycopg2 ni red.
...
# Importar desde el módulo puro, sin arrastrar dependencias de BD
...
class TestVerifyEcdsaSignature:
    ...
    def test_invalid_signature_tampered_merkle(self):
        ...
        tampered_root = _fake_merkle_root()
    ...
    def test_wrong_key(self):
        """Firma con clave A, verifica con clave B → False (suplantación de cámara)."""
    ...
    def test_urlsafe_padding_variants(self):
        ...
        Cubre todas las variantes de padding base64url.
        """
        private_key, pub_pem = _generate_keypair()
        for _ in range(20):
    ...
    def test_tampered_signature_bytes(self):
        """Firma válida con el último byte modificado → False."""
        private_key, pub_pem = _generate_keypair()
        merkle_root = _fake_merkle_root()
        signature = _sign_merkle_root(private_key, merkle_root)
    ...
    def test_invalid_pem_returns_false(self):
        """PEM corrupto → False sin excepción."""
