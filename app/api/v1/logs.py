...
def _collect_events(
    db, event_type, camera_id, user_id, date_from, date_to, current_user=None
) -> list:
    """
...
def get_activity_log(
    event_type: Optional[str] = Query(
        None,
        description="verification | video_started | video_finished | segment_uploaded",
    ),
    result: Optional[str] = Query(
        None, description="Filtrar verifs por resultado: pass | fail | error"
    ),
    camera_id: Optional[str] = Query(None, description="Filtrar por camera_id físico"),
    user_id: Optional[str] = Query(
        None, description="Filtrar por user ID (solo eventos 'verification')"
    ),
    ...
    all_events = _collect_events(
        db, None, camera_id, user_id, date_from, date_to, current_user
    )
...
def export_activity_log(
    ...
    events = _collect_events(
        db, event_type, camera_id, user_id, date_from, date_to, current_user
    )
