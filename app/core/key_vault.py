...
Estrategia de autenticación (DefaultAzureCredential):
  ...
  3. Local (development):
     ...
     → Si AZURE_KEY_VAULT_URL está configurado, DefaultAzureCredential intentará
       autenticación por az CLI, VS Code, etc.
...
Ref: OWASP ASVS §6.4.1 (gestión de claves), NIST SP 800-57 Part 1.
─────────────────────────────────────────────────────────────────────────────
"""
...
class KeyVaultClient:
    ...
    def _init_clients(self) -> None:
        ...
        try:
            ...
            logger.warning(
                ...
                "Instala: pip install azure-identity azure-keyvault-secrets azure-keyvault-keys"
            )
    ...
    def set_secret(self, name: str, value: str) -> bool:
        ...
        try:
            ...
            logger.error("Error almacenando secreto '%s' en Key Vault: %s", name, exc)
            return False
    ...
    def delete_secret(self, name: str) -> bool:
        ...
        try:
            ...
            logger.error("Error eliminando secreto '%s' de Key Vault: %s", name, exc)
            return False

    # ── Claves ECDSA por cámara ─────────────────────────────────────────────
    ...
    def get_camera_private_key(self, camera_id: str) -> Optional[str]:
        ...
        Returns:
            Clave privada en formato PEM, o None si no existe / Key Vault no disponible.
        """
        secret_name = _camera_private_key_name(camera_id)
    ...
    def store_camera_private_key(self, camera_id: str, private_key_pem: str) -> bool:
        ...
        """
        secret_name = _camera_private_key_name(camera_id)
        return self.set_secret(secret_name, private_key_pem)
    ...
    def get_camera_public_key(self, camera_id: str) -> Optional[str]:
        ...
        Convención de nombre: evideth-camera-{camera_id}-public-key
        """
        secret_name = _camera_public_key_name(camera_id)
        return self.get_secret(secret_name)
    ...
    def store_camera_public_key(self, camera_id: str, public_key_pem: str) -> bool:
        ...
        """
        secret_name = _camera_public_key_name(camera_id)
        return self.set_secret(secret_name, public_key_pem)
...
# Acceso directo para imports simples: `from app.core.key_vault import kv`
kv = get_key_vault()
